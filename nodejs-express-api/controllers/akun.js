...
import exportListPage from '../exports/akun_list.js';
...
/**
 * Route to list akun records
 * @GET /akun/index/{fieldname}/{fieldvalue}
 */
router.get(['/', '/index/:fieldname?/:fieldvalue?'], async (req, res) => {  
	try{
		...
		if(search){
			let searchFields = DB.Akun.searchFields();
		...
		if(req.query.export){
			query.attributes = DB.Akun.exportListFields();
			let records = await DB.Akun.findAll(query);
			return exportListPage(records, req, res)
		}
		query.attributes = DB.Akun.listFields();
		let page = parseInt(req.query.page) || 1;
		let limit = parseInt(req.query.limit) || 10;
		let result = await DB.Akun.paginate(query, page, limit);
		...
		if(uploadedPaths.length){
			uploadedPaths.forEach(function (fpath){
				...
				csv.fromPath(fpath, {headers: true, ignoreEmpty: true}).on("data", function(data){
					...
					try{
						let affectedRows = await DB.Akun.bulkCreate(records);
...
router.get('/view/:recid', async (req, res) => {
	try{
		...
		query.raw = true;
		query.where = where;
		query.attributes = DB.Akun.viewFields();
...
, async function (req, res) {
	try{
		...
		let usernameCount = await DB.Akun.count({ where:{ 'username': modeldata.username } });
		if(usernameCount > 0){
			return res.badRequest(`${modeldata.username} already exist.`);
		}
		
		// check if email already exist.
		let emailCount = await DB.Akun.count({ where:{ 'email': modeldata.email } });
		if(emailCount > 0){
		...
		//save Akun record
		let record = await DB.Akun.create(modeldata);
...
router.get('/edit/:recid', async (req, res) => {
	try{
		...
		query.raw = true;
		query.where = where;
		query.attributes = DB.Akun.editFields();
		let record = await DB.Akun.findOne(query);
		if(!record){
			return res.notFound();
		}
...
, async (req, res) => {
	try{
		...
		// check if username already exist.
		let usernameCount = await DB.Akun.count({where:{'username': modeldata.username, 'id': {[DB.op.ne]: recid} }});
		if(usernameCount > 0){
			return res.badRequest(`${modeldata.username} already exist.`);
		}
...
/**
 * Route to delete Akun record by table primary key
 * Multi delete supported by recid separated by comma(,)
 * @GET /akun/delete/{recid}
 */

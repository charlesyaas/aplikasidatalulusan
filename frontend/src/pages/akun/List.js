...
const AkunListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/akun/view/${data.id}`) },
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/akun/edit/${data.id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-akun`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label="Select a file to import" uploadPath="akun/importdata" buttonIcon="pi pi-folder" buttonLabel="Import Data" onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	return (
<main id="AkunListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Akun"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/akun/add`}>
                        <Button label="Add New Akun" icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                        </Link>
                    </div>
